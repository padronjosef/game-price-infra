...
```
Internet --> Nginx (SSL, ports 80/443) --> Next.js Web --> NestJS API --> PostgreSQL (Docker)
```
...
PostgreSQL runs locally in Docker (no RDS). Deploy key is generated by Terraform and passed via user-data (no Secrets Manager). AMI is pinned as a variable in tfvars (gitignored).
...
nukaloot-infra/
  ...
  terraform/
    ...
    rds.tf                    # (deprecated — DB now runs in Docker)
